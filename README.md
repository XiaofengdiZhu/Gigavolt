...
| 48\~66(0x30\~42) | 批量一元计算器  | 对左右索引范围内所有数据分别进行一元数学运算，原本的数据作为运算的输入，下端从小到大对应的运算方式分别为正弦、余弦、正切、余切、正割、余割、反正弦、反余弦、反正切、双曲正弦、双曲余弦、双曲正切、角度转弧度、弧度转角度、原码正负转换、补码正负转换、取反、加一、减一；输入和输出最高位为1时代表负数（原码模式），较高的17\~31位为整数部分，较低的1\~16位为小数部分 |
...
和存储器相似，但从二维扩展到了四维，右端口较低的16位用来指定X坐标，右端较高的16位用来指定Y坐标，左端口较低的16位用来指定Z坐标，左端较高的16位用来指定W坐标。初始化时，必须先指定四个维度的范围。同时提供了易失版（即退出游戏后数据会丢失，不过ID保持不变）  
...
用于获取玩家的各种信息，对于为小数的输出，最高位为1时代表负数（原码模式），较高的17\~31位为整数部分，较低的1\~16位为小数部分
#### 端口定义 Input Definition
| 端口 | 作用                          |
|----|-----------------------------|
| 右端 | 输出结果                        |
...
| 下端 | 较高的16位用于指定玩家序号，较低的16位用于同步操作 |
#### 下端同步操作 Sync Operation by Bottom Input
...
| 48    | 初始化图像捕获 | 要捕获玩家看到的图像，需要先输入此电压来初始化，此时将全部输出0xFFFFFFFF V；要关闭则再次输入此电压，此时将全部输出7V | /        | /          | /         |
| 49    | 捕获图像    | 将玩家看到的图像输出到由后端输入电压指定ID的存储板，需要先初始化，无输出电压                           | /        | /          | /         |

> **注意** 捕获玩家看到的图像非常消耗性能，请非必要时关闭捕获
...
下端是输入端，上和后是输出端，最高位为1时代表负数（原码模式），较高的17\~31位为整数部分，较低的1\~16位为小数部分，默认为弧度制  
提供以下电路板：
