...
用于获取玩家的各种信息，对于为小数的输出，最高位为1时代表负数（原码模式），第17\~31位为整数部分，第1\~16位为小数部分
#### 端口定义 Input Definition
| 端口 | 作用                          |
|----|-----------------------------|
| 右端 | 输出结果                        |
...
| 3     | 身体速度    | 均为小数                                                              | X        | Y          | Z         |
| 4     | 眼睛位置    | 均为小数                                                              | X        | Y          | Z         |
| 5     | 眼睛方向    | 均为小数，角度制                                                          | 偏航角（Yaw） | 俯仰角（Pitch） | 翻滚角（Roll） |
| 6     | 身体位置的信息 | 均为整数                                                              | 温度       | 湿度         | 亮度        |
| 7     | 出生点位置   | 均为小数                                                              | X        | Y          | Z         |
...
下端是输入端，上和后是输出端，最高位为1时代表负数（原码模式），第17\~31位为整数部分，第1\~16位为小数部分，默认为弧度制  
提供以下电路板：
