...
| 16~31(0x10~1F) | 左批量计算器 | 对左右索引范围内所有数据分别进行二元数学运算，原本的数据作为运算的左值，后端输入数据作为运算的右值，下端输入从小到大对应的运算方式分别为加法、减法、乘法、除法、取余、等于、大于、大于等于、小于、小于等于、取大、取小、左移、右移、乘方、对数（左真右底） |
| 32~47(0x20~2F) | 右批量计算器 | 和左批量计算器相似，只是原本的数据作为运算的右值，后端输入数据作为运算的左值                                                                                        |
| 48(0x30)       | 设置宽    | 设置被图片、地层显示器读取时显示的宽度，默认为0                                                                                                      |
| 49(0x31)       | 设置高    | 设置被图片、地层显示器读取时显示的高度，默认为0                                                                                                      |
...
下端是输入端，上和后是输出端，最高位为1时代表负数，第17~31位为整数部分，第1~16位为小数部分，默认为弧度制，负数表示为原码模式
提供以下电路板：
