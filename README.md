...
| 跳线    | 背端-标签，下端-输入，上端-输出，左端-是否从下端输入，右端-是否从其他跳线输入，具体：背端输入大于0V的电压时，将与其他背面输入一样电压的跳线相连接；当左端输入电压大于7V时，跳线会将下端输入的电压输出到该跳线连接的其他跳线；当右端输入电压大于7V时，跳线将会从连接的其他跳线获取其下端输入的电压，如果其他跳线的左端输入小于7V，则获取不到这个跳线的下端电压；从下端和连接的其他跳线获取的电压在进行或运算后从上端输出 |
|8数字LED灯| 和7段显示器类似，但它一次显示8位16进制数，只提供白光                                                                                                                                                                                       |
|方块值板| 其上有方块时，会输出等于最后一块放上的方块值的电信号，一旦所有方块离开，输出立即变为0V                                                                                                                                                                       |
|简单方块展示板| 输入等于存储了方块数据的存储板ID的电压，就会在其面前显示方块                                                                                                                                                                                    |
|简单图片显示器| 输入等于存储了图形数据的存储板ID的电压，就会在其面前显示图片                                                                                                                                                                                    |
|地层显示器| 输入等于存储了地层数据的存储板ID的电压，就会在其面前显示地层（CPU占用高，请勿显示过大的地层）                                                                                                                                                                  |
## 复杂方块 Complex Blocks
...
### 复杂方块展示板
和简单方块展示板类似，但它可以控制绘制的方块的大小、位置、旋转、亮度、颜色，具体端口定义如下（从左到右为高位到低位）
<table>
    <thead align="center">
        <tr>
            <th>端口</th>
    ...
    <tbody align="center">
        <tr>
            <td rowspan=2>后端</td>
            <td colspan="4">32位 要显示的方块值</td>
        </tr>
        <tr>
            <td colspan="4">最低10位为方块ID，14位以上为方块数据</td>
        </tr>
        <tr>
            <td rowspan="2">上端</td>
        ...
        <tr>
            <td colspan="2">每加1向上移动0.1格，最高位为1时取负，即移动范围为±3276.8格</td>
            <td colspan="2">每加1缩放增加0.1格大小，最大6553.6倍</td>
        </tr>
        <tr>
            <td rowspan="2">右端</td>
        ...
        <tr>
            <td colspan="2">每加1向西移动0.1格</td>
            <td colspan="2">每加1向北移动0.1格</td>
        </tr>
        <tr>
            <td rowspan="2">下端</td>
            <td>4位 亮度</td>
        ...
        <tr>
            <td colspan="4">控制方块的颜色，ABGR格式</td>
        </tr>
    </tbody>
</table>
