...
| 箱子提取器-指定槽位 | 用于从箱子、熔炉、发射器等可存储物品的方块中提取物品。提取器的形状是T字形，横线的一端摆放要取出物品的箱子，另一端如不摆放箱子，提取器将尝试把物品丢出；如摆放箱子，则向其存入提取的物品；也可以摆放延长轴，可以无延迟地远距离传送物品，另外延长轴是双向的。如果在两个箱子间摆放两个相对的提取器，那么这两个箱子将能够双向传送物品。需要使用电路来控制提取器，每次电压变化都将尝试进行提取，如果有多个输入，会将它们进行或运算。<br />对于指定槽位版提取器，输入电压最低8位用于指定要提取的槽位，接下来较高的8位用于指定数量，再接下来1位如果为1，则忽略指定的数量，直接提取该槽位全部的物品 |
| 箱子提取器-指定ID | 和指定槽位版本不同之处在于：输入电压最低10位用于指定要提取的方块ID；接下来较高的1位如果为1，则同时判断方块特殊值；接下来1位如果为1，则提取全部，否则提取1个；接下来2位无作用；接下来剩余的18位用于设置判断用的方块特殊值                                                                                                                                                                                  |
