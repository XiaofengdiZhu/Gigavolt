...
<table>
    ...
    <tbody align="center">
        ...
        <tr>
            <td colspan="2">每加1，显示向上移动1/8格，最高位为1时取负，即移动范围为±4096格</td>
        ...
        <tr>
            <td colspan="2">每加1，显示向西移动1/8格</td>
            <td colspan="2">每加1，显示向北移动1/8格</td>
        </tr>
        <tr>
            <td rowspan="2">下端</td>
            <td>4位 亮度</td>
        ...
        <tr>
            <td>值越大越亮，越小越暗</td>
            <td>从0变为1时弹出提示</td>
            <td>从高到低的3位分别控制偏航角、俯仰角、翻滚角的正负，为1时取负</td>
...
| 电压      | 操作     | 说明                                                                                                                            |
|---------|--------|-------------------------------------------------------------------------------------------------------------------------------|
| 无       | 异步读    | 下端无输入时，上端实时输出右索引位置的数据                                                                                                         |
| 0、48及以上 | 无      | 下端输入0时所有输出均输出0，且一旦下端有输入电压（含0V），该电路板将进入同步工作模式，输出不会随其他输入电压改变而立即发生改变                                                             |
| 1       | 读      | 读取右索引位置的数据输出到上端                                                                                                               |
| 2       | 写      | 将后端输入数据覆写到左右索引范围内（含两个索引位置，左索引为0时只覆写到右索引，下同）                                                                                   |
| 3       | 插      | 将后端输入数据插入到左右索引范围内                                                                                                             |
| 4       | 删      | 将左右索引范围内数据删除（均为0时删除索引0的数据，左索引为0时只删除右索引位置数据）                                                                                   |
| 5       | 顺查     | 从左右索引中较小的索引开始，按顺序查找后端输入数据，最多查找到较大的索引，最后将找到的索引位置输出到上端（未找到则输出0xFFFFFFFF；左右索引均为0时从头找到尾）                                          |
| 6       | 倒查     | 从左右索引中较大的索引开始，按倒序查找后端输入数据，最少查找到较小的索引，最后将找到的索引位置输出到上端（未找到则输出0xFFFFFFFF；左右索引均为0时从尾找到头）                                          |
| 7       | 查删     | 将左右索引范围内等于后端输入数据的数据删除                                                                                                         |
| 8       | 复制粘贴   | 将右索引的数据复制粘贴到左索引位置                                                                                                             |
| 9       | 剪切粘贴   | 将右索引的数据剪切粘贴到左索引位置                                                                                                             |
| 10      | 复制插入   | 将右索引的数据复制插入到左索引位置                                                                                                             |
| 11      | 剪切插入   | 将右索引的数据剪切插入到左索引位置                                                                                                             |
| 12      | 反转     | 将所有数据顺序反转                                                                                                                     |
| 13      | 升序     | 将所有数据从小到大重新排列                                                                                                                 |
| 14      | 降序     | 将所有数据从大到小重新排列                                                                                                                 |
