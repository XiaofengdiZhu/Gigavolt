...
| 开始位数 | 使用位数 | 说明                                                     |
|------|------|--------------------------------------------------------|
| 1    | 10   | 速度：单位格/秒，最大设置1024格/秒                                   |
| 11   | 7    | 角度1：面向上方或下方时，为向北方偏转的角度；面向其他方向时，为向右边偏转的角度；超过90时自动下调为90度 |
| 18   | 7    | 角度2：面向上方或下方时，为向西方偏转的角度；面向其他方向时，为向上方偏转的角度；超过90时自动下调为90度 |
| 25   | 1    | 为1时角度1变为负数                                             |
| 26   | 1    | 为1时角度2变为负数                                             |
| 27   | 1    | 为1时可设置只发射指定槽位的方块                                       |
| 28   | 3    | 指定槽位：指定只发射第n+1个槽位的方块                                   |
### 告示牌 Sign
