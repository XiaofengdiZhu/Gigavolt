...
namespace Game {
    public class SoundGeneratorGVElectricElement : RotateableGVElectricElement {
        ...
        public SoundGeneratorGVElectricElement(SubsystemGVElectricity subsystemGVElectricity, GVCellFace cellFace, uint subterrainId) : base(
        ...
        public override bool Simulate() {
                    ...
                    if (inInput > 0) {
                        if (GVStaticStorage.GVMBIDDataDictionary.TryGetValue(inInput, out GVArrayData GVMBData)) {
                            ...
                            try {
                                ...
                                m_sound = new Sound(new SoundBuffer(shorts, startIndex, itemsCount, 2, MathUint.ToIntWithClamp(leftInput)));
                            }
                            catch (Exception ex) {
                                string error = $"{CellFaces[0].Point}的GV声音发生器加载ID为{inInput.ToString("X", null)}的存储器中的音频数据时出错";
                                foreach (ComponentPlayer componentPlayer in SubsystemGVElectricity.Project.FindSubsystem<SubsystemPlayers>(true)
                                    .ComponentPlayers) {
                                    componentPlayer.ComponentGui.DisplaySmallMessage(error, Color.White, true, true);
                                }
                                Log.Error(
                                    $"{error}，加载起始位置为{startIndex}（均为十进制），加载short数量为{itemsCount}，声道数为2，采样率为{MathUint.ToIntWithClamp(leftInput)}，详细报错：\n{ex}"
                                );
                            }
                        }
                        else {
                    ...
                    if (m_sound != null) {
                        ...
                        if (m_lastBottomInput == 0) {
                            ...
                            m_sound.Volume = m_volume
                                * m_subsystemAudio.CalculateVolume(
                                    m_subsystemAudio.CalculateListenerDistance(new Vector3(cellFace.X, cellFace.Y, cellFace.Z)),
                                    0.5f + 5f * m_volume
                                );
                            m_sound.Play();
                            m_playing = true;
            ...
            if (m_playing && (DateTime.Now - m_lastNoiseTime).TotalSeconds > 1) {
                ...
                GVCellFace cellFace = CellFaces[0];
                m_subsystemNoise.MakeNoise(
                    new Vector3(cellFace.X, cellFace.Y, cellFace.Z),
                    m_volume < 0.5f ? 0.25f : 0.5f,
                    MathUtils.Lerp(2f, 20f, m_volume)
                );
            }
            return false;
        }
