...
namespace Game
{
    public class SubsystemGV8x4LedGlow : Subsystem, IDrawable
    {
        ...
        {
            var glowPoint = new GV8x4GlowPoint();
                ...
                if (key.Voltage > 0)
                {
                    ...
                    if (num > 0.01f)
                    {
                        ...
                        if (num2 < m_subsystemSky.ViewFogRange.Y)
                        {
                            ...
                            {
                                batch.QueueQuad(p, p2, p3, p4, new Vector2(1f, 1f), new Vector2(0f, 1f), new Vector2(0f, 0f), new Vector2(1f, 0f), Color.White);
                            }
                            else
                            {
                                TexturedBatch3D newBatch = generateBatch(m_primitivesRenderer, key.Type, key.Voltage);
                                newBatch.QueueQuad(p, p2, p3, p4, new Vector2(1f, 1f), new Vector2(0f, 1f), new Vector2(0f, 0f), new Vector2(1f, 0f), Color.White);
        ...
        public override void Load(ValuesDictionary valuesDictionary)
        {
            ...
            batchCache = new Dictionary<uint, TexturedBatch3D>[3] {
                new Dictionary<uint, TexturedBatch3D>(),
                new Dictionary<uint, TexturedBatch3D>(),
                new Dictionary<uint, TexturedBatch3D>()
            };
        }
        ...
        public static TexturedBatch3D generateBatch(PrimitivesRenderer3D renderer, int type, uint voltage)
        {
            ...
            return renderer.TexturedBatch(texture, false, 0, DepthStencilState.DepthRead, RasterizerState.CullCounterClockwiseScissor, BlendState.AlphaBlend, SamplerState.PointClamp);
        }
    }
}