...
namespace Game
{
	public class MotionDetectorGVElectricElement : MountedGVElectricElement
	{
		...
		public MotionDetectorGVElectricElement(SubsystemGVElectricity subsystemGVElectricity, CellFace cellFace)
			: base(subsystemGVElectricity, cellFace)
		{
			m_subsystemBodies = subsystemGVElectricity.Project.FindSubsystem<SubsystemBodies>(throwOnError: true);
			m_subsystemMovingBlocks = subsystemGVElectricity.Project.FindSubsystem<SubsystemMovingBlocks>(throwOnError: true);
			m_subsystemProjectiles = subsystemGVElectricity.Project.FindSubsystem<SubsystemProjectiles>(throwOnError: true);
			m_subsystemPickables = subsystemGVElectricity.Project.FindSubsystem<SubsystemPickables>(throwOnError: true);
		...
		public override bool Simulate()
		{
			...
			if (m_voltage > 0 && voltage == 0f)
			{
				base.SubsystemGVElectricity.SubsystemAudio.PlaySound("Audio/MotionDetectorClick", 1f, 0f, m_center, 1f, autoDelay: true);
			}
			float num = 0.25f * (0.9f + 0.000200000009f * (float)(GetHashCode() % 1000));
			base.SubsystemGVElectricity.QueueGVElectricElementForSimulation(this, base.SubsystemGVElectricity.CircuitStep + MathUtils.Max((int)(num / 0.01f), 1));
		...
		public float TestPoint(Vector3 p)
		{
			...
			if (num < 64f && Vector3.Dot(Vector3.Normalize(p - (m_center - 0.75f * m_direction)), m_direction) > 0.5f && !base.SubsystemGVElectricity.SubsystemTerrain.Raycast(m_center, p, useInteractionBoxes: false, skipAirBlocks: true, delegate (int value, float d)
			{
