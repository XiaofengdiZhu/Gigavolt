...
namespace Game
{
    public class TargetGVElectricElement : MountedGVElectricElement
    {
        ...
        public TargetGVElectricElement(SubsystemGVElectricity subsystemGVElectricity, CellFace cellFace)
        ...
        public override void OnHitByProjectile(CellFace cellFace, WorldItem worldItem)
        {
            if (m_score == 0u && m_voltage == 0u)
            {
                ...
                if (distance <= 0.5f)
                {
                    m_score = (uint)((1f-distance*2f)*(float)uint.MaxValue);
                }
