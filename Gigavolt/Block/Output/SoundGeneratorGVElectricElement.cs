...
namespace Game
{
    public class SoundGeneratorGVElectricElement : RotateableGVElectricElement
    {
            ...
            : base(subsystemGVElectricity, cellFace)
        {
            ...
            m_subsystemParticles = subsystemGVElectricity.Project.FindSubsystem<SubsystemParticles>(throwOnError: true);
        ...
        public override bool Simulate()
        {
            ...
            foreach (GVElectricConnection connection in Connections)
            {
                if (connection.ConnectorType != GVElectricConnectorType.Output && connection.NeighborConnectorType != 0)
                {
                    ...
                    {
                        if (connectorDirection.Value == GVElectricConnectorDirection.In || connectorDirection.Value == GVElectricConnectorDirection.Bottom)
                        {
            ...
            if (m_lastToneInput == 0 && num4 != 0 && num != 15 && SubsystemGVElectricity.SubsystemTime.GameTime >= m_playAllowedTime)
            {
                ...
                if (num5 != 0f && !string.IsNullOrEmpty(text2))
                {
                    ...
                    SubsystemGVElectricity.SubsystemAudio.PlaySound(text2, volume, pitch, position, minDistance, autoDelay: true);
