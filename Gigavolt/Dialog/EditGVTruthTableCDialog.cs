...
namespace Game
{
    public class EditGVTruthTableCDialog : Dialog
    {
        ...
        public GVTruthTableCData m_truthTableData;

        public GVTruthTableCData m_tmpTruthTableData;

        public bool m_ignoreTextChanges;

        public EditGVTruthTableCDialog(GVTruthTableCData truthTableData, Action<bool> handler)
        {
            XElement node = ContentManager.Get<XElement>("Dialogs/EditGVTruthTableCDialog");
            LoadContents(this, node);
            m_linearPanel = Children.Find<Widget>("EditGVTruthTableCDialog.LinearPanel");
            m_gridPanel = Children.Find<Widget>("EditGVTruthTableCDialog.GridPanel");
            m_okButton = Children.Find<ButtonWidget>("EditGVTruthTableCDialog.OK");
            m_cancelButton = Children.Find<ButtonWidget>("EditGVTruthTableCDialog.Cancel");
            m_switchViewButton = Children.Find<ButtonWidget>("EditGVTruthTableCDialog.SwitchViewButton");
            m_linearTextBox = Children.Find<TextBoxWidget>("EditGVTruthTableCDialog.LinearText");
            for (int i = 0; i < 16; i++)
            {
                m_lineCheckboxes[i] = Children.Find<CheckboxWidget>("EditGVTruthTableCDialog.Line" + i.ToString());
            }
            m_handler = handler;
            m_truthTableData = truthTableData;
            m_tmpTruthTableData = (GVTruthTableCData)m_truthTableData.Copy();
            m_linearPanel.IsVisible = false;
            m_linearTextBox.TextChanged += delegate
            {
                if (!m_ignoreTextChanges)
                {
                    m_tmpTruthTableData = new GVTruthTableCData();
