...
namespace Game
{
    public abstract class GenerateGVWireVerticesBlock : Block
    {
        ...
        public virtual void GenerateGVWireVertices(BlockGeometryGenerator generator, int value, int x, int y, int z, int mountingFace, float centerBoxSize, Vector2 centerOffset, TerrainGeometrySubset subset)
        {
            ...
            SubsystemGVElectricity.GetAllConnectedNeighbors(x, y, z, mountingFace, m_GVtmpConnectionPaths);
            foreach (GVElectricConnectionPath tmpConnectionPath in m_GVtmpConnectionPaths)
            {
                if ((num4 & (1 << tmpConnectionPath.ConnectorFace)) == 0)
                {
                    ...
                    if (!(centerOffset == Vector2.Zero) || connectorDirection != GVElectricConnectorDirection.In)
                    {
                        ...
                        float num6 = (connectorDirection == GVElectricConnectorDirection.In) ? 0f : ((tmpConnectionPath.ConnectorFace == tmpConnectionPath.NeighborFace) ? (num5 + 0.03125f) : ((tmpConnectionPath.ConnectorFace != CellFace.OppositeFace(tmpConnectionPath.NeighborFace)) ? num5 : (num5 - 0.03125f)));
