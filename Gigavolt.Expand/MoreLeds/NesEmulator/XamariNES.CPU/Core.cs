...
namespace XamariNES.CPU {
    ...
    public class Core {
        ...
        /// </summary>
        readonly Dictionary<int, CPUInstruction> _cpuInstructions;
        ...
        public void Reset() {
            ...
            ushort newProgramCounter = BitConverter.ToUInt16(new[] { CPUMemory.ReadByte(0xFFFC), CPUMemory.ReadByte(0xFFFD) }, 0);
        ...
        void ADC() {
            byte value = Instruction.AddressingMode == EnumAddressingMode.Immediate ? GetOperandByte() : CPUMemory.ReadByte(ResolveAddress());
            unchecked {
                int newA = (sbyte)A + (sbyte)value + (Status.Carry ? 1 : 0);
        ...
        public void ASL() {
            ...
            byte value = Instruction.AddressingMode == EnumAddressingMode.Accumulator ? A : CPUMemory.ReadByte(address);
        ...
        public void DEC() {
            int address = ResolveAddress();
            byte value = Instruction.AddressingMode == EnumAddressingMode.Immediate ? GetOperandByte() : CPUMemory.ReadByte(address);
            unchecked {
                value--;
            }
            Status.Zero = value == 0;
        ...
        void JMP() {
            ...
            if (Instruction.AddressingMode == EnumAddressingMode.Indirect
                && (GetOperandWord() & 0xFF) == 0xFF) {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ROL() {
            byte value = Instruction.AddressingMode == EnumAddressingMode.Accumulator ? A : CPUMemory.ReadByte(ResolveAddress());
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SAX() => CPUMemory.WriteByte(ResolveAddress(), (byte)(A & X));
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ushort PopWord() => BitConverter.ToUInt16(new[] { PopByte(), PopByte() }, 0);
        ...
        int ResolveAddress() {
            switch (Instruction.AddressingMode) {
                case EnumAddressingMode.Absolute: return GetOperandWord();
                case EnumAddressingMode.Immediate: return PC + 1;
                case EnumAddressingMode.Accumulator: return A;
                case EnumAddressingMode.ZeroPage: return GetOperandByte();
                ...
                case EnumAddressingMode.IndirectIndexed:
                    ...
                    ushort targetOffset = GetWord(GetOperandByte(), true);
                    //Check special case where 0xFFFF wraps back to 0x0000
                    if (targetOffset == 0xFFFF) {
                        return Y - 1;
                    }
                    return targetOffset + Y;
                default: throw new Exception("Unknown Addressing Mode");
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void CheckBoundary(int address1, int address2) {
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ushort GetWord(int address, bool pageWrap = false) {
            if (pageWrap && (address | 0xFF) == address) {
                return BitConverter.ToUInt16(new[] { CPUMemory.ReadByte(address), CPUMemory.ReadByte(address & ~0xFF) }, 0);
            }
