...
namespace Game {
    public class ListMemoryBankGVElectricElement : RotateableGVElectricElement {
        public readonly SubsystemGVListMemoryBankBlockBehavior m_SubsystemGVMemoryBankBlockBehavior;
        public readonly SubsystemGameInfo m_subsystemGameInfo;

        public readonly GVListMemoryBankData m_data;
        public uint m_voltage;
        public uint m_lastBottomInput;

        public ListMemoryBankGVElectricElement(SubsystemGVElectricity subsystemGVElectricity, GVCellFace cellFace, int value, uint subterrainId) : base(subsystemGVElectricity, cellFace, subterrainId) {
        ...
        public override bool Simulate() {
            ...
            if (m_data.m_worldDirectory == null) {
                m_data.m_worldDirectory = m_subsystemGameInfo.DirectoryName;
                m_data.LoadData();
                if (!m_data.m_isDataInitialized) {
                    m_data.Data = new List<uint>(256);
                }
            }
            if (bottomConnected) {
    ...
    public class UintDecComparer : IComparer<uint> {
        public int Compare(uint x, uint y) => -x.CompareTo(y);
    }
}