...
namespace Game {
    public class GVCameraBlock : RotateableMountedGVElectricElementBlock {
        ...
        public override string GetDescription(int value) => GetComplex(Terrain.ExtractData(value)) ? "用于拍照，可进行非常详尽的参数调整，详见本Mod Github页面的介绍" : "输入指定的存储板ID，每次输入发生变化时，就会对着该方块面对的方向拍照，并把图像传输到指定的存储板里，分辨率为1920*1080";

        public override void GetDropValues(SubsystemTerrain subsystemTerrain, int oldValue, int newValue, int toolLevel, List<BlockDropValue> dropValues, out bool showDebris) {
            int data = Terrain.ExtractData(oldValue);
            dropValues.Add(new BlockDropValue { Value = Terrain.MakeBlockValue(Index, 0, SetComplex(data, GetComplex(data))), Count = 1 });
            showDebris = true;
        }
