...
namespace XamariNES.PPU {
    ...
    /// </summary>
    public class Memory {
        readonly IMapper _memoryMapper;
        readonly byte[] _ppuVram;
        readonly byte[] _paletteMemory;
        ...
        public void WriteByte(int offset, byte data) {
            ...
            if (offset >= 0x2000
                && offset <= 0x3EFF) // Internal VRAM
            {
                _ppuVram[VramOffsetToOffsetIndex(offset)] = data;
                return;
            }
            if (offset >= 0x3F00
                && offset <= 0x3FFF) // Palette RAM addresses
            {
        ...
        public int VramOffsetToOffsetIndex(int offset) {
            ...
            int index = (offset - 0x2000) % 0x1000;
